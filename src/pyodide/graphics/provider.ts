/**
 * Graphics module — main-thread (provider) side.
 *
 * Handles the create_canvas via.js op that requires DOM access.
 * Returns true if the op was handled, false otherwise.
 *
 * The canvas is two stacked layers:
 *   videoCanvas (bottom) — camera frames + detection overlays (_overlayFn)
 *   studentCanvas (top)  — student draws via getContext(); persists across frames
 * Both share the same logical coordinate space (lw × lh logical pixels).
 */
import { viaRegister, viaGet } from "@/bridge/viaStore";
import { notebookStore } from "@store/notebookStore";
import { pyodideStore } from "@store/pyodideStore";

export async function handleGraphicsOp(
  op: string,
  command: any,
  viaRespond: (result: any) => void
): Promise<boolean> {
  if (op === "create_canvas") {
    const { width, height } = command;

    // Auto-size: fill the available output cell width at a 4:3 aspect ratio.
    const el = (document.querySelector(".v-main") ?? document.body) as HTMLElement;
    const availableWidth = Math.max(320, el.clientWidth - 32);
    const lw = width || Math.min(availableWidth, 1280);
    const lh = height || Math.round(lw * 3 / 4);

    const dpr = window.devicePixelRatio || 1;

    function makeCanvas() {
      const c = document.createElement("canvas");
      c.width = lw * dpr;
      c.height = lh * dpr;
      c.style.position = "absolute";
      c.style.top = "0";
      c.style.left = "0";
      c.style.width = "100%";
      c.style.height = "100%";
      return c;
    }

    // Bottom layer: camera video + detection overlays.
    const videoCanvas = makeCanvas();

    // Top layer: student draws. Transparent until student draws into it.
    const studentCanvas = makeCanvas();
    studentCanvas.style.pointerEvents = "none";

    // Wrapper div registers as the display element for CanvasResult.vue.
    const wrapperDiv = document.createElement("div");
    wrapperDiv.className = "canvas-layer-wrapper";
    wrapperDiv.style.position = "relative";
    wrapperDiv.style.width = `${lw}px`;
    wrapperDiv.style.aspectRatio = `${lw} / ${lh}`;
    wrapperDiv.appendChild(videoCanvas);
    wrapperDiv.appendChild(studentCanvas);

    const displayHandle = viaRegister(wrapperDiv);
    if (pyodideStore.runningCellId) {
      notebookStore.setResult(pyodideStore.runningCellId, {
        "application/x-via-canvas": String(displayHandle),
      });
    }

    function drawBoxes(ctx: CanvasRenderingContext2D, detections: any[]) {
      if (!detections?.length) return;
      const fontSize = Math.max(14, Math.round(lw / 40));
      ctx.strokeStyle = "#00ff00";
      ctx.lineWidth = Math.max(2, Math.round(lw / 320));
      ctx.font = `${fontSize}px sans-serif`;
      ctx.fillStyle = "#00ff00";
      for (const det of detections) {
        ctx.strokeRect(det.x, det.y, det.w, det.h);
        const pct = `${Math.round(det.confidence * 100)}%`;
        const label = det.type ? `${det.type} ${pct}` : pct;
        ctx.fillText(label, det.x + 4, det.y - 6);
      }
    }

    const VISIBILITY_THRESHOLD = 0.5;
    const POSE_CONNECTIONS: [number, number][] = [
      [0,1],[1,2],[2,3],[3,7],[0,4],[4,5],[5,6],[6,8],
      [9,10],[11,12],[11,13],[13,15],[15,17],[15,19],[15,21],
      [17,19],[12,14],[14,16],[16,18],[16,20],[16,22],[18,20],
      [11,23],[12,24],[23,24],[23,25],[24,26],[25,27],[26,28],
      [27,29],[28,30],[29,31],[30,32],[27,31],[28,32],
    ];

    function drawPoseLandmarks(ctx: CanvasRenderingContext2D, pose: any[]) {
      if (!pose?.length) return;
      const dotRadius = Math.max(4, Math.round(lw / 160));
      const lineWidth = Math.max(2, Math.round(lw / 320));
      ctx.strokeStyle = "#00ff88";
      ctx.lineWidth = lineWidth;
      for (const [a, b] of POSE_CONNECTIONS) {
        const la = pose[a];
        const lb = pose[b];
        if (!la || !lb) continue;
        if ((la.visibility ?? 0) < VISIBILITY_THRESHOLD) continue;
        if ((lb.visibility ?? 0) < VISIBILITY_THRESHOLD) continue;
        ctx.beginPath();
        ctx.moveTo(la.x, la.y);
        ctx.lineTo(lb.x, lb.y);
        ctx.stroke();
      }
      ctx.fillStyle = "#ff0055";
      for (const lm of pose) {
        if (!lm || (lm.visibility ?? 0) < VISIBILITY_THRESHOLD) continue;
        ctx.beginPath();
        ctx.arc(lm.x, lm.y, dotRadius, 0, Math.PI * 2);
        ctx.fill();
      }
    }

    // Returns a DPR-scaled context on the student canvas. The scale is applied
    // once and cached so repeated calls don't double-apply the transform.
    let studentCtx: CanvasRenderingContext2D | null = null;

    // Draws to videoCanvas with an absolute DPR transform, then registers fn as
    // the per-frame overlay so the camera rAF redraws it on each new video frame.
    function drawToVideoLayer(fn: (ctx: CanvasRenderingContext2D) => void, overlayFn: (() => void) | null) {
      const ctx = videoCanvas.getContext("2d")!;
      ctx.save();
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      fn(ctx);
      ctx.restore();
      canvasController._overlayFn = overlayFn;
    }

    const canvasController = {
      _canvas: videoCanvas,
      _logicalWidth: lw,
      _logicalHeight: lh,
      _dpr: dpr,
      _overlayFn: null as (() => void) | null,
      getContext(_type: string) {
        if (!studentCtx) {
          studentCtx = studentCanvas.getContext("2d")!;
          studentCtx.scale(dpr, dpr);
        }
        return studentCtx;
      },
      clear() {
        const ctx = studentCanvas.getContext("2d")!;
        ctx.save();
        ctx.setTransform(1, 0, 0, 1, 0, 0);
        ctx.clearRect(0, 0, studentCanvas.width, studentCanvas.height);
        ctx.restore();
      },
      drawBoundingBoxes(detections: any[]) {
        drawToVideoLayer(
          (ctx) => drawBoxes(ctx, detections),
          detections?.length
            ? () => drawBoxes(videoCanvas.getContext("2d")!, detections)
            : null
        );
      },
      drawPose(pose: any[]) {
        drawToVideoLayer(
          (ctx) => drawPoseLandmarks(ctx, pose),
          pose?.length
            ? () => drawPoseLandmarks(videoCanvas.getContext("2d")!, pose)
            : null
        );
      },
      drawPoses(poses: any[][]) {
        drawToVideoLayer(
          (ctx) => { for (const pose of poses ?? []) drawPoseLandmarks(ctx, pose); },
          poses?.length
            ? () => { for (const pose of poses) drawPoseLandmarks(videoCanvas.getContext("2d")!, pose); }
            : null
        );
      },
    };
    viaRespond({ type: "handle", id: viaRegister(canvasController) });
    return true;
  }

  if (op === "draw_image") {
    const { handle, dataUrl } = command;
    const controller = viaGet(handle);
    if (controller) {
      const img = new Image();
      await new Promise<void>((resolve) => {
        img.onload = () => resolve();
        img.onerror = () => resolve();
        img.src = dataUrl;
      });
      const ctx = controller.getContext("2d");
      ctx.drawImage(img, 0, 0, controller._logicalWidth, controller._logicalHeight);
    }
    viaRespond({ type: "value", value: null });
    return true;
  }

  return false;
}
